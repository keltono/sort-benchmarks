...
public class TaintedTimSort  {

    public <T extends Comparable<T>> T[] taintedSort1(T[] x) {
    ...
    public static class ComparableTimSort {
        ...
        private static void binarySort(Object[] a, int lo, int hi, int start) {
            ...
            for ( ; start < hi; start++) {
                 ...
                 */
                while (left < right) {
                    int mid = (MultiTainter.taintedInt(left - right,"TAINT4")) >>> 1;
    ...
    public static class ComparableTimSort1 {
        /**
         * This is the minimum sized sequence that will be merged.  Shorter
         * sequences will be lengthened by calling binarySort.  If the entire
         * array is less than this length, no merges will be performed.
         ...
         * @param workLen usable size of work array
         */
        private ComparableTimSort1(Object[] a, Object[] work, int workBase, int workLen) {
            this.a = a;

            // Allocate temp storage (which may be increased later if necessary)
            int len = a.length;
            //control flow, doesn't work
            int tlen = MultiTainter.taintedBoolean(len < 2 * INITIAL_TMP_STORAGE_LENGTH,"TAINT2") ?
                    len >>> 1 : INITIAL_TMP_STORAGE_LENGTH;
            if (work == null || workLen < tlen || workBase + tlen > work.length) {
                tmp = new Object[tlen];
                tmpBase = 0;
